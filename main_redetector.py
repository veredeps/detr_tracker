...
from models import build_redetector_from_detr
...
def get_args_parser():
    ...
    parser.add_argument('--resume', default='./zoo/detr-r50-e632da11.pth', help='resume from checkpoint')
    ...
    parser.add_argument('--git_version', default=None)
    return parser
...
def main(args):
    ...
    #d = (iter(data_loader_val)).__next__()  #    for debug - shahar
    ...
    if args.resume:
        ...
        # build parameters and lr scheduler
        n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print('number of params:', n_parameters)
...
#    for k, v in vars(args).items():  #mlflow
#        log_param(k, v)
    main(args)
    if args.output_dir:
        log_artifacts(args.output_dir)  #mlflow
